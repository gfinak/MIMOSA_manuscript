...
\noindent\textbf{Marginal likelihood derivations}\\
Because our Dirichlet-multinomial is a direct extension of the beta-binomial model, the marginal likelihoods are obtained in the exact same fashion. For a non--responder, the null marginal likelihood for a given subject is:

\[
\begin{split}
Pr(\mathbf{n}_s,\mathbf{n}_u|\boldsymbol{\alpha}_u)=&\int_0^1Pr(\mathbf{n}_s,\mathbf{n}_u|\boldsymbol{\alpha}_u,\mathbf{p}_u)\pi(\mathbf{p}_u|\boldsymbol{\alpha}_u)\mathrm{d}\mathbf{p}_u\\
=&\int_0^1Pr(\mathbf{n}_s|\boldsymbol{\alpha}_u,\mathbf{p}_u)Pr(\mathbf{n}_u|\boldsymbol{\alpha}_u,\mathbf{p}_u)\pi(\mathbf{p}_u|\boldsymbol{\alpha}_u)\mathrm{d}\mathbf{p}_u\\
...
where 
\[
p_i = \frac{w\cdot\mathrm{L_1}(\boldsymbol{\alpha}_{u},\boldsymbol{\alpha}_s|\mathbf{n}_s,\mathbf{n}_u)}{w\cdot\mathrm{L_1}(\boldsymbol{\alpha}_{u},\boldsymbol{\alpha}_s|\mathbf{n}_s,\mathbf{n}_u)+(1-w)\cdot\mathrm{L_0}(\boldsymbol{\alpha}_{u},|\mathbf{n}_s,\mathbf{n}_u)}
\]
For each parameter component updated in step 1 above, compute the acceptance ratio (e.g. $\alpha_{uk}$, below):
\[
\frac{\mathrm{L}(\alpha^{\text{new}}_{uk},\boldsymbol{\theta}_u,\boldsymbol{\alpha}_s|\cdots)\pi(\alpha^{\text{new}}_{uk})}{\mathrm{L}(\boldsymbol{\alpha}_u,\boldsymbol{\alpha}_s|\cdots)\pi(\alpha_{uk})}\\
\]

where $\pi$ is the prior distribution of the parameter, and $\boldsymbol{\theta}_u = \{\alpha_{uj} : (j \in \mathbb{Z}) (1\leq j \leq K) (j\neq k)\}$. Again, we have used the same exponential prior with mean 1,000 for each parameter. $\mathrm{L}$ is the complete marginal likelihood conditional on $\mathbf{z}$, defined as
\[
\mathrm{L}(\boldsymbol{\alpha}_u,\boldsymbol{\alpha}_s|\mathbf{n}_s,\mathbf{n}_u,\mathbf{z}) = \prod_i \mathrm{L}_{z_i}(\boldsymbol{\alpha}_u,\boldsymbol{\alpha}_s|\mathbf{n}_s,\mathbf{n}_u)
\]
