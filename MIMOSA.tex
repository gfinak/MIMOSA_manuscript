...
The primary ICS data set is from a  trial testing the GeoVax DNA and MVA vaccines in a prime-boost regimen with 120 individuals (98 vaccines and 20 placebo recipients, see supplementary material~\ref{supp:statpublished}). 
...
Here, we analyze a subset of the data consisting of 98 individual from the vaccinee group at two time points, day 0 and day 182. Samples on day 0 were taken just before vaccination and no response is expected there. 
The corresponding samples can be used as negative controls. 
Conversely, day 182 (26 weeks) should be close to the immunogenicity peak, and many individuals are expected to respond, for some cytokines at least. 
The data is described in Web Appendix A.
...
\end{equation*}
Under the null model, ${\cal M}_0$, there is no difference between the stimulated and un-stimulated samples, and the proportions are equal. Under the alternative model, ${\cal M}_1$, there is a difference in proportions between the two samples and the individual $i$ is a responder. In some study, such as the ICS studies used here, the proportion of positive cells is expected to only increase after stimulation, in which case the alternative model should be defined as $p_s>p_u$. This alternative parametrization is described in Web Appendix B and we refer to it as the one-sided model.
...
\label{s:estimation}
Our estimation algorithms make direct use of the marginal likelihoods, $\mathrm{L}_0$ and $\mathrm{L}_1$, obtained after integrating out the $p_{\{s,u\}i}$'s for the null and alternative models, which greatly simplify our calculations. Given the conjugacy of the priors, the marginal likelihoods $\mathrm{L}_0$ and $\mathrm{L}_1$ are available in closed forms (Web Appendix XX), and are given by,
 \begin{align*}
 \begin{split}
...
=\prod_{i=1}^P\binom{N_{ui}}{n_{ui}} \binom{N_{si}}{n_{si}}\cdot\\ &\frac{\mathrm{B}(n_{ui}+\alpha_u,N_{ui}-n_{ui}+\beta_u)}{\mathrm{B}(\alpha_u,\beta_u)}\frac{\mathrm{B}(n_{si}+\alpha_s,N_{si}-n_{si}+\beta_s)}{\mathrm{B}(\alpha_s,\beta_s)}
...
In addition, MIMOSA gave better estimates of the observed false discovery rate than competing methods (Figure~\ref{fig:HVTN065} B). These results are consistent for other cytokines (see Web Figure ~\ref{webfig:HVTN065Results}).
...
\node at (0,0) (A) {
\begin{tikzpicture}
\node[anchor=south west, inner sep=0] at (0,0) (foo) {\includegraphics[width=0.29\columnwidth]{Figures/Fluidigm_plot1.pdf}};
\begin{scope} [x={(foo.south east)},y={(foo.north west)}]
\node at (0,1) [font=\tiny\sffamily] {A} ;
\end{scope}
\end{tikzpicture}
...
\node [right=of A] (B) {
\begin{tikzpicture}
\node[anchor=south west, inner sep=0] at (0,0) (bar) {\includegraphics[width=0.29\columnwidth]{Figures/Fluidigm_plot2.pdf}};
\begin{scope} [x={(bar.south east)},y={(bar.north west)}]
...
\node [right=of B] (C) {
\begin{tikzpicture}
\node[anchor=south west, inner sep=0] at (0,0) (baz) {\includegraphics[width=0.29\columnwidth]{Figures/FluidigmFisher.pdf}};
...
For both the constrained and unconstrained simulations, MIMOSA out-performed competing methods, including Fisher's exact test, with respect to sensitivity and specificity at all values of $N$ (Figure~\ref{fig:simulations} A and Web Figure~\ref{webfig:simulations_trunc} panel A). Additionally, the estimated FDR for MIMOSA more closely reflected the nominal FDR compared to Fisher's exact test and competing methods (Figure~\ref{fig:simulations} B and Web Figure~\ref{webfig:simulations_trunc} panel B).
...
\end{align*}
where the indicator variable $z_i$ is as defined in \ref{ss:priors}, i.e. $z_i\sim\mathrm{Be}(w)$ where $w$ is the proportion of responders. As in the beta-binomial case both an EM and MCMC algorithms can be used for parameter estimation. When using a fully Bayesian approach via MCMC, we use the same priors for $\boldsymbol{\alpha}_{\{u,s\}}$ and $w$ as for the beta-binomial model. 

\subsection{Parameter estimation}
Again, to simplify the estimation problem, we make use of the marginal likelihoods that can be obtained in closed forms (see Web Appendix XX). For the null component, the marginal likelihood $\mathrm{L}_0$ is given by,
\begin{align*}
...
\node at (0,0) (A) {
\begin{tikzpicture}
    \node[anchor=south west, inner sep=0] at (0,0) (foo) {\includegraphics[width=0.4\columnwidth]{Figures/Multivariate_Sims_ROC.pdf}};
\begin{scope} [x={(foo.south east)},y={(foo.north west)}]
    \node at (0,1) [font=\tiny\sffamily] {A} ;
\end{scope}
\end{tikzpicture}
...
\node [right=of A] (B) {
\begin{tikzpicture}
   \node[anchor=south west, inner sep=0] at (0,0) (bar) {\includegraphics[width=0.4\columnwidth]{Figures/Multivariate_Sims_FDR.pdf}};
   \begin{scope} [x={(bar.south east)},y={(bar.north west)}]
       \node at (0,1) [font=\tiny\sffamily] {B} ;
\end{scope}
\end{tikzpicture}
...
Web Appendices A and B, and Web Figures A and B referenced in Sections \ref{s:data} and \ref{s:DEone} are available in the attached Web-based supplementary material.\vspace*{-8pt}
...
\section{}
\subsection{Web Appendix A: HVTN065 Vaccine Trial ICS Data Description}
...
\subsection{Web Appendix B: Constrained Beta--binomial model}
...
\subsection{Web Figure A: HVTN065 Results for Other Cytokines}
\begin{figure} %  figure placement: here, top, bottom, or page
   \centering
...
%\end{figure}
\subsection{Web Figure B: Other Simulation Results}
\begin{figure} %  figure placement: here, top, bottom, or page
   \centering
\begin{tikzpicture} [auto,node distance=0cm]
\node at (0,0) (A) {
